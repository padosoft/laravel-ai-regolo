...
The package ships **92 unit tests / 194 assertions** that run against a fake HTTP layer (`Http::fake()`), so the test suite never hits the real Regolo API and is safe to run in CI on every PR. No API key needed; no network needed; no money spent.

```bash
composer install
vendor/bin/phpunit
# OK (92 tests, 194 assertions)
```
